function initAutocomplete() {
    let addressInputs = document.querySelectorAll('.autocomplete-address');
    ...
    addressInputs.forEach(input => {
        ...
        autocomplete.addListener('place_changed', function () {
            ...
            let formGroup = input.closest('.address-form');
            if (!formGroup) return;

            formGroup.querySelector('.latitude').value = place.geometry.location.lat();
            formGroup.querySelector('.longitude').value = place.geometry.location.lng();

            // Initialize map only if map container exists
            let mapElement = formGroup.querySelector('.map-preview');
            if (mapElement) {
            ...
            formGroup.querySelector('.street_number').value = addressData.street_number;
            formGroup.querySelector('.route').value = addressData.route;
            formGroup.querySelector('.address2').value = addressData.address2;
            formGroup.querySelector('.postal_code').value = addressData.postal_code;
            formGroup.querySelector('.state').value = addressData.state;
            formGroup.querySelector('.country').value = addressData.country;
...
function toggleOtherCountryField(selectElement) {
    let otherCountryContainer = selectElement.closest(".address-form").querySelector(".other-country-container");

...
<IfModule headers_module>
  Header set X-Robots-Tag "noindex"
				...
				if ( $format == 'html' ) {
					$text = str_replace(
						...
						'<table width="100%" border="0" cellpadding="1" cellspacing="0" >
							<tr><td>
								<table width="100%" border="0" cellpadding="5" cellspacing="0">' . $pricing_fields . '</table>
							</td></tr>
						</table>',
						$text
					);
	...
	public static function get_submitted_fields( $form, $lead, $display_empty = false, $use_text = false, $format = 'html', $use_admin_label = false, $merge_tag = '', $options = '' ) {
		$field_data = '';
		if ( $format == 'html' ) {
			$field_data = '<table width="100%" border="0" cellpadding="1" cellspacing="0" ><tr><td>
					...
					if ( ( ! GFCommon::is_section_empty( $field, $form, $lead ) || $display_empty ) && ! $field->is_administrative() ) {
						switch ( $format ) {
							...
							default:
								$field_value = sprintf(
									'<tr>
                                        	<td colspan="2" style="font-size:14px; font-weight:bold; padding:7px 7px">%s</td>
	                                   </tr>
	                                   ', $field_label
								);
								break;
